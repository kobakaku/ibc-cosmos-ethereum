...
func (s *TestSuite) GetTxReceipt(ctx context.Context, chain *ethereum.EthereumChain, hash ethcommon.Hash) *ethtypes.Receipt {
...
func (s *TestSuite) GetRelayerUsers(ctx context.Context) (ibc.Wallet, ibc.Wallet) {
	...
	ethUsers := interchaintest.GetAndFundTestUsers(s.T(), ctx, s.T().Name(), sdkmath.NewInt(2*ethereum.ETHER), eth)

	cosmosUserFunds := sdkmath.NewInt(1_000_000_000_000)
