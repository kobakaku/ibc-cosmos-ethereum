...
import {ICS02Client} from "solidity-ibc/ICS02Client.sol";
import {ICS26Router} from "solidity-ibc/ICS26Router.sol";
import {ICS20Transfer} from "solidity-ibc/ICS20Transfer.sol";
import {TestERC20} from "solidity-ibc-test/mocks/TestERC20.sol";
import {AcceptAllSP1Verifier} from "solidity-ibc-test/mocks/AcceptAllSP1Verifier.sol";
import {Strings} from "@openzeppelin/utils/Strings.sol";
import {ICS20Lib} from "solidity-ibc/utils/ICS20Lib.sol";
...
contract MockE2ETestDeploy is Script {
    ...
    string public constant E2E_FAUCET =
    ...
    function run() public returns (string memory) {
        ...
        IICS07TendermintMsgs.ConsensusState memory trustedConsensusState = abi
            .decode(
        ...
        json.serialize(
            "ics07Tendermint",
            Strings.toHexString(address(ics07Tendermint))
        );
        json.serialize(
            "ics02Client",
            Strings.toHexString(address(ics02Client))
        );
        json.serialize(
            "ics26Router",
            Strings.toHexString(address(ics26Router))
        );
        json.serialize(
            "ics20Transfer",
            Strings.toHexString(address(ics20Transfer))
        );
        json.serialize("escrow", Strings.toHexString(ics20Transfer.escrow()));
        string memory finalJson = json.serialize(
            "erc20",
            Strings.toHexString(address(erc20))
        );
